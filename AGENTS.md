...
- Every answer is served by **bunnyCompute** (volunteer machines running small models). Users chat on **usage** first: free usage refills daily, bunny pro adds monthly usage, and both show only as percentages. **Usage is not Carrots** — never call an allowance "Carrots" or a "Carrot allowance" (internally allowances are Carrot-denominated, but never expose that). **Carrots are the backup credit** spent from the balance once usage runs out, at a flat per-answer price that depends on the model (currently 10 free / 20 pro, NOT publishable). New accounts get a Carrot starter grant (currently 1,000, NOT publishable). Spend order: free usage → pro usage → Carrot balance.
