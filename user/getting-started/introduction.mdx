...
bunny is powered by **small models**: fast, efficient AI models compact enough to run on everyday computers. They used to be too limited to be useful on their own. That has changed. Today's small models can reason, use tools, and take action, and that is exactly what makes bunny possible.

Because the model is small, it can run right on your own device, not only in a distant data center. That single fact shapes everything about how bunnyOS works.
...
- **In the bunnyCompute cloud.** bunny runs on **bunnyCompute**, a network of GPUs supplied by other people. Each request spends Carrots.
...
- **Connect your apps and tool servers.** Link accounts like Gmail and Calendar, or connect remote tool servers, so bunny can act across them. See [Connect apps & tool servers](/user/chat/connectors-and-servers).
- **Run mini-apps.** Launch **bunnyApps**, small purpose-built apps that run inside bunnyOS. See [bunnyApps & the App Store](/user/apps/app-store).
